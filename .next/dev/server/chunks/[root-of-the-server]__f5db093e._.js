...
"[project]/app/api/metrics/analysis/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET() {
    try {
        const metrics = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$db$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getMetrics"](5000);
        if (metrics.length === 0) {
        ...
        const timeSeriesData = metrics.slice(0, 24).reverse().map((m)=>({
        ...
        const cpuPowerCorrelation = metrics.slice(0, 24).reverse().map((m)=>({
                time: new Date(m.timestamp_utc).toLocaleTimeString(),
        ...
        metrics.forEach((m)=>{
            ...
            if (!computerStatsMap.has(computerName)) {
                computerStatsMap.set(computerName, {
                    watts: [],
                    maxWatts: 0
                });
            }
        ...
        const computerStats = Array.from(computerStatsMap.entries()).map(([computer_name, stats])=>({
                computer_name,
                avgWatts: Math.round(stats.watts.reduce((a, b)=>a + b, 0) / stats.watts.length * 100) / 100,
        ...
        const idleAnalysis = Array.from(computerStatsMap.entries()).map(([computer_name, stats])=>({
                computer_name,
                idleHours: Math.floor(Math.random() * 12),
...
//# sourceMappingURL=%5Broot-of-the-server%5D__f5db093e._.js.map